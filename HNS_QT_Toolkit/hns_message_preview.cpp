...
int HNS_Message_Preview::fInsertMessage(const QString &multi, const int &pos, const bool &do_update)
{
    ...
    if(index >= 0 && index < f_page_strings.size())
    {
        ...
        {
            //f_page_strings.insert(index+1,temp_pages.data());
...
void HNS_Message_Preview::paintEvent(QPaintEvent */*event*/)
{
    ...
    //HNS_SignBoard signboard = f_current_message.fGetPage(f_current_page).fGetSignBoard();
    HNS_SignBoard signboard = HNS_Message2::fGetDefaultSignBoard();
    ...
    else if(signboard.fGetType() == HNS_BRD_TRAILER_CHARACTER_BOARD)
    {
        ...
        for(size_t i=0;i<signboard.fGetBoardsWide()-1;i++)
        {
            right = (((width() * width_ratio)/signboard.fGetBoardsWide()) + ((width() * (1 - width_ratio))/(signboard.fGetBoardsWide() - 1))) * (i+1);
            left = right - ((width() * (1 - width_ratio))/(signboard.fGetBoardsWide() - 1));
            top = 0;
            bottom = height();
            painter.fillRect(QRect(QPoint(left,top),QPoint(right,bottom)),QBrush(Qt::white,Qt::SolidPattern));
        }

        for(size_t i=0;i<signboard.fGetBoardsTall()-1;i++)
        {
            bottom = (((height() * height_ratio)/signboard.fGetBoardsTall()) + ((height() * (1 - height_ratio))/(signboard.fGetBoardsTall() - 1))) * (i+1);
            top = bottom - ((height() * (1 - height_ratio))/(signboard.fGetBoardsTall() - 1));
            left = 0;
            right = width();
            painter.fillRect(QRect(QPoint(left,top),QPoint(right,bottom)),QBrush(Qt::white,Qt::SolidPattern));
        }
        ...
        if(f_images.size() > 0)
        {
            ...
            {
                row = ui / signboard.fGetBoardsWide();
                col = ui % signboard.fGetBoardsWide();

                xpos = (((width() * width_ratio)/signboard.fGetBoardsWide()) + ((width() * (1 - width_ratio))/(signboard.fGetBoardsWide() - 1))) * col;
                ypos = (((height() * height_ratio)/signboard.fGetBoardsTall()) + ((height() * (1 - height_ratio))/(signboard.fGetBoardsTall() - 1))) * row;
                box_width = ((width() * width_ratio)/signboard.fGetBoardsWide());
                box_height = ((height() * height_ratio)/signboard.fGetBoardsTall());

                QImage preview = f_images[ui].scaled(box_width,box_height);
                test_height = preview.height();
                test_width = preview.width();
                preview.save(QString("test_scaled%1.bmp").arg(ui),"bmp");
...
void HNS_Message_Preview::fUpdate(const bool &refresh_pages, const bool &blank)
{
    string tempstring;
    ...
    {
        signboard = f_current_message.fGetPage(f_current_page).fGetSignBoard();
        if(signboard.fGetType() == HNS_BRD_TRAILER_FULL_MATRIX)
        {
            if(!blank)
            {
        ...
        else if(signboard.fGetType() == HNS_BRD_TRAILER_CHARACTER_BOARD)
        {
            ...
            if(!blank)
            {
                ...
                for(size_t ui = 0; ui < signboard.fGetNumBoards(); ui++)
                {
                    ...
                    f_images[ui].save(QString("test%1.bmp").arg(ui),"bmp");
                }
            }
        }
    }
